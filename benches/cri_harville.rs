...
fn criterion_benchmark(c: &mut Criterion) {
    ...
    //         b.iter(|| {
    //             old_harville_summary_no_alloc(&dilated_probs, k, &cardinalities, &mut podium, &mut bitmap, &mut summary);
    //         });
    //     });
    // }
    fn bench(c: &mut Criterion, n: usize, k: usize) {
        ...
        c.bench_function(&format!("cri_harville_{n}x{k}"), |b| {
            b.iter(|| {
                harville_summary_no_alloc(&dilated_probs, k, &mut podium, &mut bitmap, &mut summary);
            });
        });
    }
    bench(c, 3, 3);
