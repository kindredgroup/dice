...
fn criterion_benchmark(c: &mut Criterion) {
    fn bench(c: &mut Criterion, n: usize, k: usize, degree: usize) {
        ...
        let mut alloc = mass_samp::Alloc::new(n, k);
        
        c.bench_function(&format!("cri_harville_mass_samp{n}x{k}_d{degree}"), |b| {
            b.iter(|| {
                mass_samp::summary_no_alloc(&dilated_probs, degree, &mut alloc);
            });
        });
    }
    bench(c, 6, 6, 3);
